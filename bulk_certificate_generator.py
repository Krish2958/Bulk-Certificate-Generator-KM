...
def process_ppt_and_generate_pdfs(template_ppt_path, csv_path, output_dir):
    ...
    if not os.path.isfile(template_ppt_path):
        raise FileNotFoundError("Selected PPT template does not exist.")
    ...
    for index, row in data.iterrows():
        ...
        # Create a unique file name for each output
        ...
        if convert_ppt_to_pdf(os.path.abspath(output_pptx_path), os.path.abspath(output_pdf_path)):
            # Remove the PPTX file after successful conversion
            os.remove(output_pptx_path)
            print(f"Deleted PPTX: {output_pptx_path}")
        else:
            print(f"Failed to convert PPTX to PDF: {output_pptx_path}")
...
def run_process():
    try:
        if not template_ppt_path or not csv_path or not output_dir:
